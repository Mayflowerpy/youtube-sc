...
def transcribe_audio():
    ...
    try:
        ...
        for segment in segments:
            ...
            if segment_count % 50 == 0:  # Log every 50 segments
                current_time = segment.end
                percent = min((current_time / info.duration) * 100, 100) if hasattr(info, 'duration') else 0
                log.info(f"Processed {segment_count} segments - Current time: {current_time:.1f}s ({percent:.1f}%)")
