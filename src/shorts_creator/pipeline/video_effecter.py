...
def _apply_aspect_ratio_conversion(video, conversion_info: dict, target_w: int, target_h: int):
    if conversion_info['is_wide']:
        log.info("Wide video: scaling and centering vertically")
        return video.filter('scale', target_w, -1).filter('pad', target_w, target_h, 0, '(oh-ih)/2', 'black')
    else:
...
def _apply_title_animation(video, title_text: str, title_y_position: int, title_duration: float, font_path: str):
...
def _apply_static_subscribe_text(video, subscribe_y_position: int, video_duration: float, font_path: str):
...
def __basic_effects(
    ...
    try:
        ...
        title_y_position = _calculate_title_position(conversion_info['black_bar_top_height'])
